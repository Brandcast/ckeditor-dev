...
function dataIsEmpty( data )
{
	...
	if(isPlainText) {
		...
		return !$(value).toArray().map(function(node) { return node.textContent; }).join('');
	}
}
...
function removePlaceholder(ev) {
	...
	if (editor.mode == 'wysiwyg' )
	{
		if (!root.hasClass( 'show-placeholder' ))
			return;

		root.removeClass( 'show-placeholder' );
		// fill it properly
	...
	init : function( editor )
	{
		...
		if (placeholder)
		{
			...
			editor.on( 'getData', function( ev ) {
				...
				if ( element && element.hasClass( 'show-placeholder' ) )
					ev.data.dataValue = '';
			});
